 ...
 * 
 * 处理目标：
 * - Fragment的android:name
 * - Activity的android:name（少见）
 * 
...
public class NavigationProcessor {
    ...
    private static final Logger log = LoggerFactory.getLogger(NavigationProcessor.class);
    
    private final SemanticValidator semanticValidator;
    private final ClassMapping classMapping;
    private final PackageMapping packageMapping;
    
    public NavigationProcessor(SemanticValidator semanticValidator,
    ...
    public byte[] process(String filePath, byte[] xmlData) throws IOException {
        ...
        log.info("处理navigation文件: {}", filePath);
        ...
        try {
            ...
            if (replaceCount[0] == 0) {
                log.info("navigation文件处理完成: {}, 替换0处（未修改，返回原始数据）", filePath);
                return xmlData;
            }
            ...
            log.info("navigation文件处理完成: {}, 替换{}处", filePath, replaceCount[0]);
            return result;
            
        } catch (Exception e) {
            log.error("navigation文件处理失败: {}", filePath, e);
            throw new IOException("navigation文件处理失败: " + e.getMessage(), e);
        }
    }
}
