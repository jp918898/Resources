...
/**
 * Menu文件Visitor - 工业级AXML层级结构保护
 * 
 * 功能：
 * - 替换app:actionViewClass属性
 * - 替换app:actionProviderClass属性
 * - 自动维护XML层级结构（由AxmlReader管理栈）
 * 
 * @author Resources Processor Team
 * @version 1.0.0
 */
public class MenuVisitor extends NodeVisitor {
    
    private static final Logger log = LoggerFactory.getLogger(MenuVisitor.class);
    ...
    public MenuVisitor(NodeVisitor child,
    ...
    public void attr(String ns, String name, int resourceId, int type, Object obj) {
        ...
        if (!Objects.equals(obj, newObj)) {
            replaceCount[0]++;
            log.debug("替换menu属性: {} = '{}' -> '{}'", name, obj, newObj);
        }
    ...
    private Object replaceAttributeIfNeeded(String attrName, Object attrValue) {
        ...
        // 只处理actionViewClass和actionProviderClass
        if (!"app:actionViewClass".equals(attrName) && 
            !"app:actionProviderClass".equals(attrName)) {
            return attrValue;
        }
